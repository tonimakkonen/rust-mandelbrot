...
impl Shader {
    pub fn from_source(name_and_content: (&str, &str)) -> Shader {
        let id = shader_from_file(name_and_content);
        Shader { id }
    }
    pub fn id(&self) -> gl::types::GLuint {
        self.id
...
pub fn shader_from_file(name_and_content: (&str, &str)) -> gl::types::GLuint {
    ...
    let res = shader_from_source(&CString::new(name_and_content.1).unwrap(), kind);
    return res.unwrap();
}
